import type { DraftCredentialItem } from "@credets/shared-types/credentials/listings";
...
import { getDraftListings } from "./-actions/getDraftListings";

export const Route = createLazyFileRoute("/credentials/draft/")({
...
function DraftItemCard({ item }: { item: DraftCredentialItem }) {
	...
	return (
		...
		>
			<div className="flex items-center gap-4 rounded-xl border bg-card px-4 py-3.5 shadow-xs transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md hover:border-amber-200/50 dark:hover:border-amber-800/30 active:translate-y-0 active:shadow-xs">
				{/* Icon */}
				<div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-amber-100/80 ring-1 ring-amber-200/60 dark:bg-amber-950/30 dark:ring-amber-800/40">
				...
				<div className="flex min-w-0 flex-1 items-center gap-3">
					<div className="min-w-0 flex-1">
						<h3 className="font-semibold text-base leading-snug truncate tracking-tight group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors duration-200">
							{item.title || "Untitled"}
						</h3>
						<div className="flex items-center gap-2 mt-0.5">
							...
							{/* Favourite indicator */}
							{item.is_favourite && (
								<span className="inline-flex items-center gap-1 text-[10px] text-rose-400/60">
		...
		<div className="flex flex-col items-center justify-center py-24">
			<div className="relative mb-8">
				<div className="size-28 rounded-full bg-gradient-to-br from-amber-200/50 via-amber-100/30 to-muted flex items-center justify-center ring-1 ring-amber-200/40 dark:from-amber-900/30 dark:via-amber-800/20 dark:ring-amber-700/30">
					<FileEdit className="size-12 text-amber-400/60 dark:text-amber-500/50" />
				</div>
			</div>
...
function DraftErrorUI({ error }: { error: Error }) {
	return (
	...
	return (
		<CredentialsShell>
			<div className="mx-auto w-full max-w-3xl px-4 py-8">
				...
				{/* Error state */}
				{!isLoading && isError && <DraftErrorUI error={error as Error} />}

				{/* Empty state */}
				{!isLoading && !isError && items.length === 0 && <DraftEmptyState />}
				...
				{!isLoading && !isError && items.length > 0 && (
					<p className="text-xs text-muted-foreground/40 text-center mt-6">
						{items.length} draft credential{items.length !== 1 ? "s" : ""}
					</p>
				)}
			</div>
			...
			<AlertDialog open={deleteAllOpen} onOpenChange={setDeleteAllOpen}>
				<AlertDialogContent>
					...
					<div className="mt-4 rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-xs text-destructive/70">
						All draft credentials will be permanently removed. Make sure you
