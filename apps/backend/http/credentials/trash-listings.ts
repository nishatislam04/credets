...
import type { CursorPayload } from "../../repository/credentials/trash-listings";
import { getTrashListingsService } from "../../services/credentials/trash-listings";

export async function trashListings(req: BunRequest) {
	try {
		...
		const result = await getTrashListingsService({ limit, cursorResult });
		...
		return ResponseFactory.success({
			...
			path: req,
			message: "trash listings fetched",
			status: 200,
		});
	} catch (error) {
		logger(error, "http: error in trashListings controller");

		if (error instanceof AppError) {
			return ResponseFactory.error({
				error: error.message,
				type: error.type,
				message: error.message || "Failed to fetch trash listings",
				status: error.status,
				path: req,
				data: {},
			});
		...
		return ResponseFactory.error({
			...
			message:
				error instanceof Error
					? error.message
					: "Failed to fetch trash listings",
			status: 500,
			path: req,
			data: {},
			details: {
...
function parseCursorParam(
	...
	if (
		...
		return ResponseFactory.error({
			error: "bad request",
			message:
				"Invalid cursor payload: must contain 'deletedAt' and 'id' fields",
			status: 400,
			path: req,
		});
	}
