...
export async function getDraftListingsRepo(
	limit: number,
	cursorResult: CursorPayload | null,
): Promise<DraftRow[]> {
	try {
		if (cursorResult) {
			return await sql<DraftRow[]>`
				SELECT
		...
		return await sql<DraftRow[]>`
			...
			FROM credentials
			LEFT JOIN types t ON credentials.types_id = t.id
			WHERE credentials.is_draft = true AND credentials.is_deleted = false
			ORDER BY credentials.created_at DESC, credentials.id DESC
			LIMIT ${limit + 1}
		`;
	} catch (error) {
