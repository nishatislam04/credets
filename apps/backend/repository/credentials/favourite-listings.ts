...
export async function getFavouriteListingsRepo(
	...
	try {
		...
		return await sql<FavouriteRow[]>`
			...
			FROM credentials
			LEFT JOIN types t ON credentials.types_id = t.id
			WHERE credentials.is_favourite = true AND credentials.is_deleted = false
			ORDER BY credentials.created_at DESC, credentials.id DESC
			LIMIT ${limit + 1}
		`;
	} catch (error) {
		logAlways(error, "repo: favourite listings query failed");
